...
def fetch_batch(segment: str, chunk: list[int], batch_idx: int, total_batches: int, today_date: datetime.date, id_to_symbol: dict) -> str:
    """Fetch and process a batch of symbols with improved error handling."""
    ...
    try:
        ...
        if candles:
            for c in candles:
                session.merge(c)  # Use merge to handle updates if record exists
            session.commit()
            
        elapsed = time.time() - start_time
        return f"[OK] {segment} batch {batch_idx}/{total_batches} - Processed {processed}, inserted {len(candles)} in {elapsed:.2f}s"
...
def fetch_today_eod_data(today_date: datetime.date, max_workers: int = 5):
    """Fetch today's EOD data for all active symbols with improved monitoring."""
    # Ensure tables exist
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, future in enumerate(as_completed(futures)):
                ...
                if (i + 1) % 5 == 0 or (i + 1) == len(futures):
                    ...
                    log(f"[PROGRESS] {i+1}/{len(futures)} batches, Success: {successful}, Failed: {failed}, "
